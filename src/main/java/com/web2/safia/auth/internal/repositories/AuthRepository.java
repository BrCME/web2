package com.web2.safia.auth.internal.repositories;
...
public interface AuthRepository extends JpaRepository<User, UserId> {
	@NativeQuery("""
			SELECT
				u.*,
				(u.deleted_at IS NULL) AS non_expired,
				(u.status != 'BLOCKED') AS non_locked,
				(u.status != 'BLOCKED' AND u.deleted_at IS NULL) AS enabled
			FROM auth."user" u
